...
namespace
{
    const char* get_image_origin_name(vkShade::TrackedImageOrigin origin)
    {
        switch (origin)
        {
    ...
    const char* get_image_role_name(const vkShade::TrackedImageSnapshot& image)
    {
    ...
    std::string get_image_usage_string(VkImageUsageFlags usage)
    {
        struct UsageName
        {
...
void vkShade::BufferPanel::render()
{
    if (m_trackedImages.empty() || ImGui::GetTime() >= m_nextRefresh)
    {
        try
        {
            m_trackedImages = m_imageTracker->get_snapshot();
        }
    ...
    while (clipper.Step())
    {
        for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row)
        {
            ...
            ImGui::TextUnformatted(get_image_role_name(image));
            ImGui::TableSetColumnIndex(2);
            ImGui::TextUnformatted(get_image_origin_name(image.origin));
            ...
            const std::string usage = get_image_usage_string(image.usage);
