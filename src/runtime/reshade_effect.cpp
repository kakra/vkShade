...
void vkShade::ReshadeEffect::reflect_pipeline()
{
    for (auto&& [pass, passInfo] : std::views::zip(m_passes, m_module->techniques[0].passes))
    {
        ...
        if (colorFormats.empty())
            colorFormats.push_back(VulkanImage::view_format(m_swapchainInfo.format,
                                                            passInfo.srgb_write_enable));
...
void vkShade::ReshadeEffect::reflect_uniforms()
{
    ...
    for (const auto& uniform : m_module->uniforms)
    {
        ...
        metadata.size = uniform.size,
        metadata.offset = uniform.offset,
        metadata.baseType = convert_uniform_type(uniform.type),
        metadata.components = uniform.type.components(),
        metadata.uiLabel = uniform.name;
        ...
        for (const auto& annotation : uniform.annotations)
        {
                ...
                annotation.type.base == reshadefx::type::t_string)
            {
                if (!annotation.value.string_data.empty())
                    metadata.uiType = convert_uniform_ui_type(annotation.value.string_data);
            }
            else if (annotation.name == "ui_min")
            {
                metadata.uiMin = getNumericValue(annotation);
            }
            else if (annotation.name == "ui_max")
            {
                metadata.uiMax = getNumericValue(annotation);
            }
            else if (annotation.name == "ui_step")
            {
                metadata.uiStep = getNumericValue(annotation);
            }
                ...
                annotation.type.base == reshadefx::type::t_string)
            {
                for (size_t pos = 0, end; pos < annotation.value.string_data.size(); pos = end + 1)
                {
                    ...
                    if (end > pos)
                        metadata.uiItems.emplace_back(annotation.value.string_data, pos, end - pos);
                }
            }
                ...
                annotation.type.base == reshadefx::type::t_string)
            {
                ...
                if (!annotation.value.string_data.empty())
                    metadata.uiLabel = annotation.value.string_data;
            }
            else if (annotation.name == "ui_tooltip" &&
                annotation.type.base == reshadefx::type::t_string)
            {
                metadata.uiTooltip = annotation.value.string_data;
            }
            else if (annotation.name == "ui_category" &&
                annotation.type.base == reshadefx::type::t_string)
            {
                metadata.uiCategory = annotation.value.string_data;
            }
            else if (annotation.name == "ui_category_closed" &&
                annotation.type.base == reshadefx::type::t_bool)
            {
                metadata.uiCategoryClosed = annotation.value.as_uint[0] != 0;
            }
            else if (annotation.name == "ui_category_toggle" &&
                annotation.type.base == reshadefx::type::t_bool)
            {
                metadata.uiCategoryToggle = annotation.value.as_uint[0] != 0;
            }
            else if (annotation.name == "ui_text" &&
                annotation.type.base == reshadefx::type::t_string)
            {
                metadata.uiText = annotation.value.string_data;
            }
            else if (annotation.name == "ui_spacing")
            {
                if (annotation.type.base == reshadefx::type::t_int)
                    metadata.uiSpacing = annotation.value.as_int[0];
                else if (annotation.type.base == reshadefx::type::t_uint)
                    metadata.uiSpacing = static_cast<int32_t>(annotation.value.as_uint[0]);
            }
            else if (annotation.name == "ui_units" &&
                annotation.type.base == reshadefx::type::t_string)
            {
                metadata.uiUnits = annotation.value.string_data;
            }
            else if (annotation.name == "hidden" &&
                annotation.type.base == reshadefx::type::t_bool)
            {
                metadata.hidden = annotation.value.as_uint[0] != 0;
            }
            else if (annotation.name == "noedit" &&
                annotation.type.base == reshadefx::type::t_bool)
            {
                metadata.disabled = annotation.value.as_uint[0] != 0;
            }
            else if (annotation.name == "noreset" &&
                annotation.type.base == reshadefx::type::t_bool)
            {
                metadata.noReset = annotation.value.as_uint[0] != 0;
            }
        }
