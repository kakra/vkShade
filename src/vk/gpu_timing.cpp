...
void vkShade::GpuTiming::collect_results()
{
    ...
    m_state->totalGpuMilliseconds.store(
        get_milliseconds(results[TotalBegin].value, results[TotalEnd].value),
        std::memory_order_relaxed);
    m_state->effectsGpuMilliseconds.store(
        get_milliseconds(results[EffectsBegin].value, results[EffectsEnd].value),
        std::memory_order_relaxed);
...
double vkShade::GpuTiming::get_milliseconds(uint64_t begin, uint64_t end) const
{
